...
def main():
    experiments = ['fish_baseline_concurrent', 'fish_baseline_concurrent_universalization']

    for experiment in experiments:
        seeds = generate_random_seeds(5)
        for seed in seeds:
            group_name = f'Llama-2-13b-chat-hf_{experiment}'
            llm_path = 'meta-llama/Llama-2-13b-chat-hf'
            change_config('simulation/conf/config.yaml', group_name, llm_path, seed)

            run_experiment('Llama-2-13b-chat-hf', seed, experiment)
